...
#H0: ??distanceMRT = 0, the distance to the nearest metro rail transit (in meters) has no effect on the house price of unit area.
#Ha: ??distanceMRT NOT equal 0, the distance to the nearest metro rail transit (in meters) has an effect on the house price of unit area.
...
#On average, the estimated intercept value(??), may differ from the actual value by approximately 59.20875 units.
#On average, the estimated value(??) for the slope parameter, may differ from the true value by approximately 0.03557 units.

CI_level <- confint(model1)
CI_level

#95% Confidence interval for ??(intercept) [lower bound = 4302.7439928, upper bound = 4535.5235074]
...
#95% Confidence interval for ?? (slope) [lower bound = -0.7650898, upper bound = -0.6252367]
...
#Model 1 -The adjusted squared value is 0.5858, the model explains about 58.58% of the variation in the dataset is explained by the model. The residual standard eror is 817.1, this implies that on average, the predicted values deviate from the true regression line by 817.1. The F-statistic is 145.3, with a small p-value, meaning it has at least one predictor that has a significant effect on the responses of Price.
...
# i. Fit a polynomial regression model to predict price using distanceMRT of order 3 and testthe model significance. Give the resulting model.
...
# ii. Construct a scatter plot to visualize the relationship between price and log transformedvalues of distanceMRT (price vs log(distanceMRT)).
